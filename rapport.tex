...
Ce rapport présente les solutions aux exercices de la \textbf{Leçon 5 -- Structural Patterns} :
implémentation en Python du \textbf{Composite} (arbre du menu), implémentation d'un itérateur récursif
(concept Iterator) et exemple \textbf{Head First} (Decorator — Starbuzz Coffee).
Le dépôt contient les sources, diagrammes et tests. Le code important est inclus directement dans ce rapport via \texttt{\textbackslash lstinputlisting} (extraits) ; les fichiers complets sont référencés par des liens GitHub pour consultation rapide.

% -------------------------
\chapter{Introduction et contexte}
\label{ch:intro}
Les patterns de conception (Design Patterns) sont des solutions réutilisables à des problèmes récurrents en conception orientée objet.
Ce travail se concentre sur les \emph{patrons structurels} (Composite, Iterator, Decorator) illustrés dans la Leçon 5.
...
Le problème est de représenter une hiérarchie partie-tout (menus et items) où le client peut traiter uniformément les feuilles (plats) et les composites (sous-menus). L'intention du patron Composite est de composer des objets en structures arborescentes et de permettre aux clients d'interagir de façon uniforme.

\section{Modèle conceptuel appliqué au menu}
La structure implémentée contient :
\begin{itemize}
  \item \textbf{MenuComponent} : interface abstraite (API commune). (voir \texttt{src/menu_composite/menu_component.py})
  \item \textbf{MenuItem} : feuille (plat).
  \item \textbf{Menu} : composite, contenant des enfants.
  \item \textbf{Waitress} : le serveur (client) qui utilise l'API pour afficher, filtrer, et rechercher.
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{docs/diagrams/menu_example.png}
...
Le choix d'utiliser une interface commune simplifie le code client. Pour de très grands arbres, considérer une sérialisation/lazy-loading ou l'utilisation d'une base PostGIS/BD si les menus sont volumineux ou dynamiques (hors sujet ici).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{docs/diagrams/Schema-genral-iterator.png}
...
La série \emph{Head First} privilégie l'apprentissage visuel et par l'exemple. L'exemple classique du livre pour le pattern \emph{Decorator} est \emph{Starbuzz Coffee} — un objet boisson décoré dynamiquement par des condiments.

\section{Schéma}
\begin{figure}[H]
  \centering
  % si tu veux ajouter un schéma PlantUML, tu peux générer l'image et la mettre dans docs/diagrams
...
Exécution de démonstration : \texttt{src/headfirst_decorator/starbuzz\_demo.py}.
...
pytest -q
\end{verbatim}
Le jeu de tests se trouve dans \texttt{tests/test\_menu.py}.
...
\begin{itemize}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/blob/main/src/menu_composite/menu.py}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/blob/main/src/menu_composite/menu_item.py}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/blob/main/src/menu_composite/menu_component.py}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/blob/main/src/menu_composite/composite_iterator.py}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/blob/main/src/menu_composite/waitress.py}
  \item \url{https://github.com/Tiger-Foxx/design-patterns-impl/tree/main/src/headfirst_decorator}
\end{itemize}
...
\begin{itemize}
  \item Reproduction de l'arbre du menu (Composite) en Python.
...
\end{itemize}
Le dépôt contient les codes, diagrammes et tests ; le rapport intègre les extraits essentiels et référence les sources complètes sur GitHub.
